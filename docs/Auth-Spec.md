...
- **Logout** (`POST /auth/logout`): delete the current session row
  and clear the cookie. Defined in
  `packages/api/src/routes/auth-login.ts`.
- **Logout all** (`POST /auth/logout-all`): delete every session
  for this user, including the current one. Defined in
  `packages/api/src/routes/auth-sessions.ts`.
