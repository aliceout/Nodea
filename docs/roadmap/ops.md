...
- **Tâches (app-side)**
  ...
  - [ ] **Étape 2 (Sentry)** ([issue #73](https://github.com/aliceout/Nodea/issues/73)) : ajouter `@sentry/node` côté API + `@sentry/react` côté web. **Après livraison de SEC-01 (livré).** Init avec `beforeSend` qui filtre les request bodies et les query strings sensibles. ~1h. **Reporté** : tentative d'install pendant la session du pas 1 a été bloquée par un lock Windows persistant sur `@tailwindcss/oxide` qui empêche `pnpm install` de finaliser les symlinks. Le code template (init + beforeSend) est documenté dans l'issue.
