...
- **Tâches (app-side)**
  ...
  - [x] **Étape 2 (Sentry)** : `@sentry/node` ([`packages/api/src/sentry.ts`](../../packages/api/src/sentry.ts)) + `@sentry/react` ([`packages/web/src/sentry.ts`](../../packages/web/src/sentry.ts)) installés et initialisés. `beforeSend` agressif côté api ET web : strip cookies / query strings / request body / headers `x-sid` `x-guard` `cookie` `authorization` / `user.email` `user.username` `user.ip_address`. Côté web, BrowserTracing + Replay filtrés out pour ne pas leak les navigations `/flow` à Sentry. No-op quand `SENTRY_DSN` / `VITE_SENTRY_DSN` est unset.
