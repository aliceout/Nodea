...
**Préconditions** : `requireFreshPasswordOrPasskey`.

**Body** :
```json
{
  "opaque_register_request": "...",
  "opaque_register_record": "...",
  "wrapped_kek_password": "...",
  "wrapped_kek_password_iv": "..."
}
```

(Le client a fait OPAQUE registration en parallèle d'une opération
serveur ; l'export_key est dérivé localement, utilisé pour
re-wrapper la KEK existante. La main key et la KEK ne changent pas.)

**Serveur** (transaction) :
1. Vérifie OPAQUE registration record et stocke (remplace l'ancien
   envelope).
2. UPDATE `users.wrapped_kek_password{,_iv}` avec les nouveaux blobs.
3. **Rotation de l'ID de session** : DELETE toutes les sessions de
   cet user (incluant la courante). INSERT une nouvelle session
   `kind = 'full'` avec `reauth_password_at = now()` (l'user vient
   de prouver son nouveau password).
4. Émet un nouveau cookie `__Host-nodea_session` signé. L'ancien
