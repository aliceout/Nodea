...
| Backup chiffré portable | `age` (passphrase) — scrypt + ChaCha20-Poly1305 | scrypt logN=18 ; ChaCha20-Poly1305 clé 256 bits ; phrase BIP39 12 mots **dérivée de la main key** (versionnée, rotatable) ; confirmée par un quiz de transcription | format `age`, librairie `age-encryption` 0.3.0 (Filippo Valsorda) — cf. ADR-0016 |
...
- `always_2fa` — 2ᵉ facteur requis après le password (TOTP **ou** passkey, au choix de l'utilisateur·ice) ; TOTP requis après passkey-first. Activable dès qu'**au moins un 2ᵉ facteur** est configuré : TOTP enrôlé **ou** au moins une passkey enrôlée.
- `maximum` — password + passkey PRF + TOTP, les trois requis. Activable seulement si TOTP enrôlé **et** une passkey PRF enrôlée.

Downgrade auto §6.1 : si l'utilisateur·ice désactive le facteur qui maintient le mode, le mode retombe à `password_or_passkey` dans la même transaction (avec email de notification). En `always_2fa`, le downgrade ne se déclenche que lorsque les **deux** facteurs alternatifs disparaissent (TOTP désactivé **et** dernière passkey supprimée) — perdre l'un en gardant l'autre ne casse pas le mode.
...
- **Action × utilisateur·ice par croisement de logs (sans le module).** Toutes les lectures, écritures, éditions, suppressions de contenu chiffré passent par un seul endpoint `/records` ; le nom du module voyage dans un header HTTP (`X-Collection`) que ni Nginx ni le logger Hono n'enregistrent **par défaut**. Un opérateur qui croise les logs Nginx (path + IP + horodatage) et la table `sessions` (user_id ↔ IP) peut reconstruire « user U a fait une opération à l'heure T » — mais plus quel module. Le contenu reste chiffré, les rows ne portent toujours pas de `user_id`, et le module n'apparaît plus dans l'attribution. **Honnêteté sur la limite** : « par défaut » fait le travail. `X-Collection` est un en-tête de requête en clair — l'opérateur du proxy TLS qui le termine *peut* le journaliser s'il le configure explicitement (Nginx `log_format` avec `$http_x_collection`, un sidecar de mirroring, etc.). De plus, le motif des tailles de blob et la cadence des LIST par collection laissent une signature statistique qui peut, sur la durée, ré-attribuer une partie du trafic à un module. La garantie est donc « le module ne fuite pas dans la configuration de logs livrée », pas « il est cryptographiquement caché de l'opérateur ».
...
En complément, un second export — la **sauvegarde chiffrée** (`.age`) — produit une sauvegarde **portable et indépendante du compte** : scellée sous une phrase BIP39 de 12 mots **dérivée de la main key** (versionnée, donc rotatable) puis notée, elle est réimportable dans un **nouveau** compte. Format `age` (un blob opaque enveloppant un ZIP d’un JSON par module), déchiffré en local à l’import après saisie de la phrase. Voir ADR-0016.

La **sauvegarde cloud automatique** (ADR-0017) pousse ce même blob `.age` directement du navigateur vers un service tiers (Dropbox, pCloud ou Nextcloud/WebDAV), au plus une fois par jour, à l’ouverture de session si la dernière date de plus de 24 h. Le serveur Nodea n’intervient à aucune étape : pas de nouvel endpoint, pas de token côté serveur. Le **credential** du service (refresh token OAuth, ou app-password Nextcloud) est scellé dans les **préférences chiffrées** (`user_preferences.payload`, champ `cloudBackup`) — donc lisible uniquement en session déverrouillée, jamais en clair côté serveur. Le service de destination ne voit que le `.age` opaque, exactement ce que l’utilisateur·ice pouvait déjà télécharger à la main.

La **restauration** parcourt le même chemin en sens inverse : téléchargement du `.age` → déchiffrement local avec la phrase → fusion **non-destructive** (dédup par clé naturelle de chaque module, on n’ajoute que ce qui manque, jamais d’écrasement ni de suppression). La phrase est re-dérivée et essayée silencieusement ; ce n’est qu’en cas d’échec (sauvegarde d’un autre compte / autre version de phrase) que les 12 mots sont demandés. Garde-fou : **créer** une sauvegarde (export manuel `.age`/`.json`, push manuel ou auto vers le cloud) est verrouillé tant que la phrase n’est pas confirmée par le quiz `/backup` ; la **restauration** et la simple **connexion** d’un service restent ouvertes (entrée, et la phrase est re-dérivable). Voir ADR-0017.
