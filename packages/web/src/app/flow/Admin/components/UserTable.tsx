...
export default function UserTable({ users, currentUserId, onDelete }: UserTableProps) {
  ...
  if (users.length === 0) {
    return <EmptyHint>{t('admin.userTable.empty')}</EmptyHint>;
  }
          ...
          >
            <div className="min-w-0 flex-1">
              <p className="text-[13.5px] font-medium text-ink">
                {user.username ?? (
                  <span className="italic text-muted">{t('admin.userTable.noName')}</span>
                )}
              </p>
            ...
            {isSelf ? (
              <span className="text-[11px] italic text-muted">{t('admin.userTable.self')}</span>
            ) : (
              <Button
