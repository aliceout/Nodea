...
import ChartBody from '@/app/flow/Mood/components/ChartBody';
...
/**
 * Homepage snapshot of the last 20 Mood entries — a compact sparkline
 * plus the rolling average. The data layer (`useMoodTrend`) belongs to
 * the Mood module; this file is the Homepage's public surface for it.
 */
export default function MoodOverview({ module }: MoodOverviewProps) {
  const Icon = module?.icon;
  const { status, data, error } = useMoodTrend({ latestEntries: RECENT_ENTRIES_LIMIT });
  const { t } = useI18n();
  ...
  const averageLabel = average !== null ? average.toFixed(1).replace(/\.0$/, '') : '—';

  let message = '';
  if (status === 'loading') {
    message = t('home.mood.loading', { defaultValue: 'Chargement…' });
    ...
    message = t('home.mood.empty', { defaultValue: 'Aucune donnée pour le moment.' });
  }
