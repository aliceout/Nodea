...
const startRoute = createRoute({
  ...
  request: {
    body: {
      content: {
        'application/json': { schema: ChangePasswordStartBodySchema },
      },
    },
  },
  responses: {
    200: jsonContent(ChangePasswordStartResponseSchema, 'OPAQUE registration response + token'),
...
const finishRoute = createRoute({
  ...
  request: {
    body: {
      content: {
        'application/json': { schema: ChangePasswordFinishBodySchema },
      },
    },
  },
  responses: {
    200: okContent('Password rotated, session re-minted'),
    400: errorContent('Invalid body'),
    401: errorContent('Invalid credentials or token mismatch'),
  },
});
 ...
 */
authChangePasswordRoutes.openapi(startRoute, async (c) => {
  await opaqueReady;
 ...
 * either way.
 */
authChangePasswordRoutes.openapi(finishRoute, async (c) => {
  const raw = await c.req.json().catch(() => null);
