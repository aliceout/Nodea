...
| 4 | Front : refonte du noyau crypto | done (legacy .js deleted in Step 6) | `base64`, `hkdf`, `aes`, `hmac`, `key-material`, `guard-derivation`, `argon2`, `envelope` in TS with branded types. HKDF domain separation (`nodea:aes` / `nodea:hmac`). `wipeMainKeyMaterial` placebo gone. Guards cache in-memory only. Legacy `webcrypto.js` / `main-key.js` / `guards.js` / `crypto-utils.js` deleted. |
...
| 10 | Déploiement Docker + extinction PocketBase | done | Full docker-compose (postgres + api + web). API Dockerfile runs migrations on start then boots the Hono server. Web Dockerfile multi-stage (vite build → nginx) with a reverse-proxy `/api/` → `api:3000`. README rewritten around `docker compose up -d --build`. PocketBase entirely deleted in Step 6 (`config/pocketbase/`, `install.sh`, `pocketbase` dep). |
...
- [x] **MOYENNE** — Inconsistent README install (Phase 10) — rewritten around `docker compose up -d --build`; quickstart, dev loop, and admin seeding all documented.
